<meta charset="utf-8" />
<meta http-equiv="x-ua-compatible" content="ie=edge" />
<title>ShopGrids - @yield('title')</title>
<meta name="description" content="" />
<meta name="viewport" content="width=device-width, initial-scale=1" />
<link rel="shortcut icon" type="image/x-icon" href="{{ asset('shopgrids/assets/images/favicon.svg') }}" />

<link rel="stylesheet" href="{{ asset('shopgrids/assets/css/bootstrap.min.css') }}" />
<link rel="stylesheet" href="{{ asset('shopgrids/assets/css/LineIcons.3.0.css') }}" />
<link rel="stylesheet" href="{{ asset('shopgrids/assets/css/tiny-slider.css') }}" />
<link rel="stylesheet" href="{{ asset('shopgrids/assets/css/glightbox.min.css') }}" />
<link rel="stylesheet" href="{{ asset('shopgrids/assets/css/main.css') }}" />
